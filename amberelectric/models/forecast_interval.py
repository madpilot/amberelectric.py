...
class ForecastInterval(BaseModel):
    ...
    __properties = ["type", "duration", "spotPerKwh", "perKwh", "date", "nemTime", "startTime", "endTime", "renewables", "channelType", "tariffInformation", "spikeStatus", "descriptor", "range", "advancedPrice"]

    @validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('ForecastInterval'):
            raise ValueError("must be one of enum values ('ForecastInterval')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ForecastInterval:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ForecastInterval:
        """Create an instance of ForecastInterval from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ForecastInterval.parse_obj(obj)

        _obj = ForecastInterval.parse_obj({
            "type": obj.get("type"),
