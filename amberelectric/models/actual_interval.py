...
class ActualInterval(BaseModel):
    ...
    __properties = ["type", "duration", "spotPerKwh", "perKwh", "date", "nemTime", "startTime", "endTime", "renewables", "channelType", "tariffInformation", "spikeStatus", "descriptor"]

    @validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('ActualInterval'):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ActualInterval:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ActualInterval:
        """Create an instance of ActualInterval from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ActualInterval.parse_obj(obj)

        _obj = ActualInterval.parse_obj({
            "type": obj.get("type"),
